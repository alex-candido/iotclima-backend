# django_app/modules/v1/events/api.py
...
from .serializers import EventsInputSerializer, EventsOutputSerializer
...
class EventsViewSet(viewsets.ViewSet):
    service: EventsService = core_container.events_container.service()
    ...
    def list(self, request):
        ...
        serializer = EventsOutputSerializer(page, many=True)
        return self.pagination_class.get_paginated_response(serializer.data, total_count=total_count)
    
    # POST /events/
    def create(self, request):
        input_data = self._validated_data(EventsInputSerializer, data=request.data)
        output = self.service.create(input_data)
        data = self._to_response(EventsOutputSerializer, output)
    ...
    def update(self, request, pk=None):
        ...
        input_data = self._validated_data(EventsInputSerializer, data=request.data, instance=instance, partial=False)
    ...
    def partial_update(self, request, pk=None):
        instance = self.service.get_instance(pk)
        input_data = self._validated_data(EventsInputSerializer, data=request.data, instance=instance, partial=True)
        output = self.service.update(pk, input_data)
        data = self._to_response(EventsOutputSerializer, output)
        return Response(data)
