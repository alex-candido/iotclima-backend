# django_app/modules/v1/records/api.py
...
from .serializers import RecordsInputSerializer, RecordsOutputSerializer
...
class RecordsViewSet(viewsets.ViewSet):
    ...
    def create(self, request):
        input_data = self._validated_data(RecordsInputSerializer, data=request.data)
        output = self.service.create(input_data)
        data = self._to_response(RecordsOutputSerializer, output)
    ...
    def update(self, request, pk=None):
        ...
        input_data = self._validated_data(RecordsInputSerializer, data=request.data, instance=instance, partial=False)
    ...
    def partial_update(self, request, pk=None):
        instance = self.service.get_instance(pk)
        input_data = self._validated_data(RecordsInputSerializer, data=request.data, instance=instance, partial=True)
        output = self.service.update(pk, input_data)
        data = self._to_response(RecordsOutputSerializer, output)
        return Response(data)
